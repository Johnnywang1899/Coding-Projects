...
double Ki = 0.015;                                        // integral gain in [ms^-1]
double Kd = 2000;                                          // derivative gain in [ms]
...
void PID_Control() {
  ...
  */
  ...
  if (t_now - t_last_PID >= T_sample){      // if the elapsed time is greater than the sampling time, time to send control signal
    ...
    setpoint_angle = 15*sin(0.62831852*(T_set/1000))+30;
    T_set = T_set + T_sample;

//    setpoint_angle = T_set * 0.005;
//    if (j < 900 ){
//      T_set += 10;
//     }else if (j < 1800){
//      T_set -= 10;
...
//    Serial.println("");   
    Serial.println ("Setpoint angle is: ");
